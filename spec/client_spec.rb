...
RSpec.describe RescueGroupsV5::Client do
    ...
    let(:opts) do
      {
        filters: filter_opts,
        fields: {
          animals: %i[name breedPrimary],
          orgs: %i[name city state]
        },
        include: [:orgs],
        postalcode: "92107",
        radius_in_miles: 100,
  ...
  describe "#get_animal" do
    ...
    let(:opts) do
      {
        fields: {
          animals: %i[name breedPrimary ageGroup]
        }
      }
    end
    let(:subject) do
  ...
  describe "#search_orgs" do
    ...
    it "returns orgs filtered by attributes" do
      ...
      uniq_city_names = city_names.map(&:downcase).uniq
    ...
    let(:opts) do
      {
        filters: filter_opts,
        fields: {
          animals: %i[breedPrimary ageGroup name]
        }
      }
    end
    let(:subject) do
